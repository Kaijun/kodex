<!-- KDX OVERLAY RULES START -->
# KDX Downstream Overlay

These rules govern the Kaijun/kodex downstream overlay and take precedence for KDX maintenance work.
...
- When `main` is regenerated, preserve the latest upstream `AGENTS.md` and inject only this marked KDX block. Do not freeze or replace upstream instructions outside this block.
...
- Keep downstream changes declarative in `alias/kdx.json`, `alias/scripts/apply-alias.py`, `alias/scripts/render-install.py`, `alias/scripts/install.template.sh`, and their tests. Do not commit generated or patched upstream Rust source files.
...
- Update `alias/scripts/test_apply_alias.py` whenever patch behavior changes. Run patch tests, installer rendering, and shell syntax checks before compilation.
