  ...
  Linux)
    case "$arch" in
      ...
      aarch64 | arm64)
        ...
        legacy_target="aarch64-unknown-linux-musl"
        platform_label="Linux arm64"
        ;;
      *)
        echo "${display_name} on Linux currently supports x86_64 and arm64 only (detected: ${arch})." >&2
        exit 1
        ;;
    esac
    ;;
  *)
    echo "${display_name} currently supports macOS arm64 and Linux (x86_64, arm64) only." >&2
    exit 1
    ;;
esac
    ...
    case "$platform" in
      ...
      linux-arm64)
        case "$file_output" in
          ...
          *) echo "Release binary ${binary} is not a Linux arm64 executable." >&2; exit 1 ;;
        esac
        ;;
    esac
  fi
