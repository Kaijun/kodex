...
"""
            ...
            self.assertIn("supports macOS arm64 and Linux x86_64 only", res.stderr)

    def test_rejects_unsupported_linux_arm64(self):
        with tempfile.TemporaryDirectory() as probe_dir:
            bin_dir = Path(probe_dir) / "bin"
            bin_dir.mkdir()
            mock_uname = bin_dir / "uname"
...
"""
            ...
            self.assertNotEqual(res.returncode, 0)
            self.assertIn("Linux currently supports x86_64 only", res.stderr)
