...
`kdx.showCurrentProjectSessionsOnly` defaults to `true` and limits
`thread/list` requests to exact working-directory matches for the folders in the
current VS Code workspace. Multi-root workspaces pass every root to the app
server. Set it to `false` in user or workspace settings to show sessions from all
projects; an empty VS Code window also keeps the global list available.
