...
function isCliInstallSupportedPlatform() {
  return (
...
function currentProjectSessionCwds() {
  const enabled = vscode.workspace
    .getConfiguration("kdx")
    .get("showCurrentProjectSessionsOnly", true);
  if (!enabled) {
    return undefined;
  }
  const folders = vscode.workspace.workspaceFolders || [];
  const cwdValues = folders
    .map((folder) => folder?.uri?.fsPath)
    .filter((value) => typeof value === "string" && value.length > 0);
  if (cwdValues.length === 0) {
    return undefined;
  }
  return cwdValues.length === 1 ? cwdValues[0] : cwdValues;
}

function rewriteThreadListRequests(chunk, cwdValues) {
  if (cwdValues === undefined) {
    return chunk;
  }
  const binary = Buffer.isBuffer(chunk) || chunk instanceof Uint8Array;
  if (!binary && typeof chunk !== "string") {
    return chunk;
  }
  const input = binary ? Buffer.from(chunk).toString("utf8") : chunk;
  let changed = false;
  const output = input
    .split("\n")
    .map((line) => {
      const carriageReturn = line.endsWith("\r") ? "\r" : "";
      const jsonLine = carriageReturn ? line.slice(0, -1) : line;
      if (!jsonLine.trim()) {
        return line;
      }
      try {
        ...
        if (
          ...
          Object.hasOwn(request.params, "cwd")
        ) {
          return line;
        }
        request.params.cwd = cwdValues;
        changed = true;
        return `${JSON.stringify(request)}${carriageReturn}`;
      } catch {
        return line;
      }
    })
  ...
  return binary ? Buffer.from(output, "utf8") : output;
}

function installSessionListFilter(child) {
  if (!child?.stdin || typeof child.stdin.write !== "function") {
    return child;
  }
  const originalWrite = child.stdin.write;
  child.stdin.write = function filteredWrite(chunk, ...args) {
    const updated = rewriteThreadListRequests(
      chunk,
      currentProjectSessionCwds(),
    );
    return originalWrite.call(this, updated, ...args);
  };
  return child;
}

function isAppServerInvocation(command, args) {
  if (typeof command !== "string" || !Array.isArray(args)) {
    return false;
  }
  const executableName = path.basename(command).toLowerCase();
  const isConfiguredExecutable =
    typeof kdxExecutable === "string" &&
    path.resolve(command) === path.resolve(kdxExecutable);
  return (
    (isConfiguredExecutable ||
      ["kdx", "kdx.exe", "codex", "codex.exe"].includes(executableName)) &&
...
function installProcessRedirect() {
  for (const method of ["spawn", "spawnSync", "execFile", "execFileSync"]) {
    ...
    childProcess[method] = function redirected(command, ...args) {
      const result = original.call(this, redirectKdxCommand(command), ...args);
      if (method === "spawn" && isAppServerInvocation(command, args[0])) {
        return installSessionListFilter(result);
      }
      return result;
    };
  }
...
async function installCli(release) {
  if (!isCliInstallSupportedPlatform()) {
    throw new Error(
      "KDX CLI automatic installation currently supports macOS arm64 and Linux (x64, arm64) only.",
    );
  }
...
async function ensureKdxExecutable() {
  ...
  const actions = isCliInstallSupportedPlatform()
