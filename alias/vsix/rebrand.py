...
def rewrite_package_json(
    ...
    properties["kdx.showCurrentProjectSessionsOnly"] = {
        "description": (
            "Show only sessions whose working directory exactly matches a folder "
            "in the current VS Code workspace. When no folder is open, KDX shows "
...
def validate_vsix(
    ...
    with zipfile.ZipFile(vsix) as archive:
        ...
        session_filter = (
            ...
            .get("configuration", {})
            .get("properties", {})
            .get("kdx.showCurrentProjectSessionsOnly")
        )
        if session_filter != {
            "description": (
                "Show only sessions whose working directory exactly matches a folder "
            ...
            raise RuntimeError(
                "repacked extension is missing the current-project session setting"
            )
